-- The MCP page tools record their receipts in the same table as every other
-- MCP draft mutation, so the allowed operation list has to name them. SQLite
-- cannot widen a CHECK constraint in place, so the table is rebuilt and its
-- rows are carried over, the same way 0024 widened the connection scope list.
--
-- The rebuild also adds `error_reason`: the named cause behind a refusal, such
-- as `page_is_home`. A replayed refusal must say the same thing as the first
-- one, so the reason is stored beside the code and the message rather than
-- rebuilt from them.
...
INSERT INTO mcp_mutation_receipts (
  ...
  error_code, error_message, latest_revision, conflict_resource, replay_count,
  created_at
)
SELECT
  site_id, actor_id, operation, idempotency_key, input_hash, invocation_id,
