-- `foundry.blog.create` and `foundry.blog.update` write a draft revision, so
-- they record their receipts in the same table as every other MCP draft
-- mutation and the allowed operation list has to name them. The blog's own
-- archive, restore and schedule request are not draft writes: they keep their
-- own audit in `blog_post_operation_audit_events` and add no row here.
-- SQLite cannot widen a CHECK constraint in place, so the table is rebuilt and
-- its rows are carried over, the same way 0030 and 0031 did.
