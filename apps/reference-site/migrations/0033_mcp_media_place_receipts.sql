-- `foundry.media.place` writes a draft revision, so it records its receipt in
-- the same table as every other MCP draft mutation and the allowed operation
-- list has to name it. `foundry.media.list` and `foundry.media.upload` add no
-- row here: the list writes nothing, and an upload writes no draft revision
-- and keeps its own receipt and audit in the media library's own tables.
-- SQLite cannot widen a CHECK constraint in place, so the table is rebuilt and
-- its rows are carried over, the same way 0030, 0031 and 0032 did.
