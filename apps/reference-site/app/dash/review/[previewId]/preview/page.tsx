...
/**
 * Opens the canonical preview of the exact revision the review screen shows.
 *
 * The review screen links here instead of holding the preview capability in
 * its own markup. The capability is short-lived and bound to the person, the
 * workspace and the revision, so it is minted for this navigation only.
 */
export default async function McpPreviewOpenPage({
  params,
}: {
  ...
  const media = await createHumanMediaAccessToken(
    access.identity,
    revision.definition.pages.flatMap((page) =>
      (page.media ?? []).map(({ asset }) => asset.assetId),
    ),
